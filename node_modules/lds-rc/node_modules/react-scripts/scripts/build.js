...
function buildCloud(){
	...
	paths.packComponent = path.resolve(paths.pack, 'component');
	
	//生成模块包
	fs.emptyDirSync(paths.packComponent);
	let modules = {};
	let filePathJS = path.resolve(paths.appBuild, 'static', 'js');
	fs.readdir(filePathJS, function(err,files){
    if(err){
      console.log(err);
      return;
    }
    files.forEach(function(fileName){
    	...
    	let hash8 = fileName.split('.')[1];
			let packModule = path.resolve(paths.packComponent, moduleName);
			modules[moduleName] = {
				"hash8": hash8
			};
    	fs.emptyDirSync(packModule);
    })
    files.forEach(function(fileName){
    	let moduleName = fileName.split('.')[0];
    	let hash8 = fileName.split('.')[1];
			let packModule = path.resolve(paths.packComponent, moduleName);
    	fs.copySync(path.resolve(filePathJS, fileName), path.resolve(packModule, fileName.replace(hash8 + '.', '')));
    })
 	})
	let filePathCSS = path.resolve(paths.appBuild, 'static', 'css');
	fs.readdir(filePathCSS, function(err,files){
    if(err){
      console.log(err);
      return;
    }
    files.forEach(function(fileName){
    	...
    	let hash8 = fileName.split('.')[1];
			let packModule = path.resolve(paths.packComponent, moduleName);
			fs.copySync(path.resolve(filePathCSS, fileName), path.resolve(packModule, fileName.replace(hash8 + '.', '')));
			if(!fs.existsSync(path.resolve(filePathCSS, fileName))){
    		//fs.renameSync(packModule, path.resolve(paths.packComponent, moduleName + '.' + hash8));
    		modules[moduleName] = {
					"hash8": hash8
				};
			}
    })
		let packModuleInfo = path.resolve(paths.packComponent, 'module.json')
		fs.writeJSONSync(packModuleInfo, modules);
 	})
  ...
  fs.copySync(paths.appBuild, paths.packPatch, {
    ...
    filter: filePath => {
    	...
    	let pre_path = path.replace(paths.appBuild, paths.packAll);
	...
	function clearEmptyDir(dirPath) {
  	if(!fs.existsSync(dirPath)){
		...
		let files = fs.readdirSync(dirPath);
	 	files.forEach(walk);
	 	function walk(fileFullName){
	 		let filePath = path.resolve(dirPath, fileFullName);
